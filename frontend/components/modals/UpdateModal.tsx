...
export default function UpdateModal() {
  ...
  const railwayNumber = searchParams.get("railwayN");
  const trainNumber = searchParams.get("trainN");
  const departureStation = searchParams.get("depSt");
  const arrivalStation = searchParams.get("arrSt");
  ...
  const handleCreateRecord = async (formData: FormData) => {
    ...
    await updateRecord(id || "", result.data);
    router.back();
  };
  return (
    <div className="fixed inset-0 flex h-full w-full items-center justify-center overflow-y-auto bg-background/95">
      <div className="relative w-96 rounded-md p-8 shadow-lg">
        <div className="text-center">
          <h3 className="text-2xl font-bold">Update Train Record</h3>
          <div className="mt-2 px-7 py-3">
            <form className="flex flex-col gap-2.5" action={handleCreateRecord}>
              <input
                ...
                min={1}
                name="trainNumber"
                defaultValue={Number(trainNumber)}
              ...
              <input
                ...
                min={1}
                name="railwayNumber"
                defaultValue={Number(railwayNumber)}
              ...
              <input
                ...
                defaultValue={String(departureStation)}
              ...
              <input
                type="text"
                name="arrivalStation"
                defaultValue={String(arrivalStation)}
